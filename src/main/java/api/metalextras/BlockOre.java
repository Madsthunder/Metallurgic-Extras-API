...
public class BlockOre extends BlockFalling
{
	...
	{
		super(Material.ROCK);
		this.setRegistryName(material.getRegistryName().toString() + "."
				+ types.getRegistryName().toString().replaceFirst(":", "_"));
		this.property = new OreTypeProperty(types, this);
		this.setDefaultState(this.getBlockState().getBaseState());
		this.setDefaultState(this.getBlockState().getProperties().isEmpty() ? this.getDefaultState()
				: this.getDefaultState().withProperty(this.getOreTypeProperty(),
						this.getOreTypeProperty().getAllowedValues().get(0)));
	...
	public void harvestBlock(World world, EntityPlayer player, BlockPos pos, IBlockState state,
			@Nullable TileEntity entity, @Nullable ItemStack stack)
	{
		...
		else
		{
			...
			if (type.getState().getBlock().canHarvestBlock(world, pos, player))
				Block.spawnAsEntity(world, pos, new ItemStack(block.getItemDropped(type.getState(), world.rand, 0),
						block.quantityDropped(world.rand), block.damageDropped(type.getState())));
		}
	}
	...
	public List<ItemStack> getDrops(IBlockAccess access, BlockPos pos, IBlockState state, int fortune)
	{
		...
		list.add(new ItemStack(typeBlock.getItemDropped(typeState, random, 0), typeBlock.quantityDropped(random),
				typeBlock.damageDropped(typeState)));
		return list;
	}
	...
	public boolean addDestroyEffects(World world, BlockPos pos, ParticleManager manager)
	{
		...
		{
			OreType type = this.getOreType(state);
			TextureAtlasSprite texture = this.getOreMaterial().getModel(type)
					.bake(ModelRotation.X0_Y0, DefaultVertexFormats.BLOCK, ModelLoader.defaultTextureGetter())
					.getParticleTexture();
			if (texture != null)
				textures.add(texture);
			texture = type.getModel(this.getOreMaterial())
					.bake(ModelRotation.X0_Y0, DefaultVertexFormats.BLOCK, ModelLoader.defaultTextureGetter())
					.getParticleTexture();
			if (texture != null)
				textures.add(texture);
		...
		for (int j = 0; j < 4; j++)
			for (int k = 0; k < 4; k++)
				for (int l = 0; l < 4; l++)
				{
					...
					ParticleDigging particle = (ParticleDigging) new ParticleDigging.Factory().createParticle(0, world,
							x, y, z, x - pos.getX() - .5, y - pos.getY() - .5, z - pos.getZ() - .5,
	...
	public boolean addHitEffects(IBlockState state, World world, RayTraceResult result, ParticleManager manager)
	{
		...
		ParticleDigging particle = (ParticleDigging) new ParticleDigging.Factory().createParticle(0, world, d0, d1, d2,
	...
	public boolean addLandingEffects(IBlockState state, WorldServer world, BlockPos pos, IBlockState useless,
			EntityLivingBase entity, int particles)
	{
		return true;
	}
	...
	{
		return this.getBlockState().getProperties().isEmpty() ? this.property.getAllowedValues().get(0)
				: state.getValue(this.property);
	}

	public final IBlockState getBlockState(OreType type)
	{
		return this.getBlockState().getProperties().isEmpty() ? this.getDefaultState()
				: this.getDefaultState().withProperty(this.property, type);
	}
	...
	{
		return type.getTypes() == this.getOreTypeProperty().getTypes()
				? this.getBlockState().getProperties().isEmpty() ? 0 : this.property.getAllowedValues().indexOf(type)
				: -1;
	}
}
