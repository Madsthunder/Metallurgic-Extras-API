...
public abstract class OreProperties implements Predicate<IBlockState>
{
	...
	public IBlockState getOre(IBlockState state, World world, BlockPos pos)
	{
		...
		IBlockState state1 = this.id_to_iblockstate.getByValue(id);
		if(state1 != null)
			return state1;
		OreType type = OreUtils.getOreType(state);
		if(type != null)
		{
			state1 = this.id_to_iblockstate.getByValue(id);
			if(state1 == null)
			{
				state1 = this.getOreMaterial().applyBlockState(type);
				if(state1 != null)
					this.id_to_iblockstate.put(state1, id);
			}
			if(state1 != null)
				return state1;
		}*/
